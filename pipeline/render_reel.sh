...
ZO="if(lt($T,8.60),1.0,1.0+0.28*($T-8.60))"
ffmpeg -y -v error \
...
[2:v]trim=0.25:9.35,setpts=PTS-STARTPTS,fps=30,scale=1080:1920,setsar=1,$G73,scale=3240:5760,zoompan=z='$ZO':x='(iw-iw/zoom)/2':y='(ih-ih/zoom)*0.33':d=1:s=1080x1920:fps=30,format=yuv420p[vo];
...
[5:v]setpts=PTS-STARTPTS[ocap]; [x3][ocap]overlay=0:1440:eof_action=pass,format=yuv420p,fade=t=out:st=32.60:d=0.3,tpad=stop_mode=clone:stop_duration=0.45[vout];
...
" -map "[vout]" -map "[aout]" \
 ...
 -c:a aac -b:a 256k -ac 2 -movflags +faststart claude-books-flights-v11.mp4
ffprobe -v error -show_entries format=duration,size -of default=noprint_wrappers=1 claude-books-flights-v11.mp4
