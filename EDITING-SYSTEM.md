...
- **UI sound design — real recordings only (synth got rejected):**
  - Keyboard clicks ONLY when an actual keyboard/typing-person is implied on screen (someone typing a prompt). An AI response typing itself out gets NO typing sound — nobody is typing (Nathan caught this).
  - **THE typing sound (locked after 8 rounds — final rule):** a CONTINUOUS UNTOUCHED PASSAGE of a real person typing on a "creamy keyboard" (Nathan's reference: youtube.com/shorts/ptXUL5Wro2s, @dangoniko creamy-boards ASMR). Source: pipeline/sfx/keyboard-creamy-freesound.mp3. NEVER slice individual strikes and re-sequence them — every synthetic-rhythm attempt (grid-timed, human-pattern-generated, gain-jittered kits) got rejected; only real passages have real rhythm.
  - Passage selection: scan the recording for a window that COVERS THE FULL visible typing span, first key <0.2s in, last key near the end, no gap >0.55s. Edge fades 0.12s in / 0.30s out, peak ~0.4 (sits under the voice). Verify per-half-second bed energy across the whole typing window.
  - Sourcing sounds like these: Mixkit has NO usable keyboard/ASMR audio (bright office clicks; lowpassing makes boops). Freesound search is a JS app — render it in the proxied Chromium (capture_app.Browser), pull [data-mp3] preview URLs from the DOM, fetch with requests. Search terms that worked: "creamy keyboard", "creamy typing keyboard". YouTube reference audio can't be ripped headless (playback blocked) — identify the sound signature from the title/comments and match from licensed recordings.
