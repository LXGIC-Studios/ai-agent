...
- **UI sound design — real recordings only (synth got rejected):**
  ...
  - **THE typing sound (locked after a 6-round tasting):** chunky-milky-thocky ASMR keys — pipeline/sfx/thock-kit-final.wav (5 matched Gazzew Boba strikes harvested from a real enthusiast recording, pipeline/sfx/keyboard-thock-gazzew-freesound.mp3). Recipe: harvest ALL strikes, keep only ring-free ones (tail-tonality screen — typewriter/notification samples hide BEEPS), pick the 5 closest to the median character (mixed-character kits sound "weird"), milk them (0.17s window, long decay fade), muffle (LP ~2.9kHz), soften attack (5ms ramp), peak ~0.38 so they sit UNDER the voice.
  - **Typing rhythm is HUMAN, not the text-update grid:** clicks follow the sentence — bursts per word, gaps between words, 1-2 thinking pauses — pattern normalized to fit the visible typing window. Clicking on every 30fps text change reads as machine-gun (Nathan: "like a real person typing, it needs a rhythm").
  - Mixkit has NO usable keyboard/ASMR sounds (bright office clicks only; lowpassing them makes boops). Freesound previews work: render freesound.org search in the proxied Chromium (capture_app.Browser), pull [data-mp3] URLs from the DOM, fetch with requests.
